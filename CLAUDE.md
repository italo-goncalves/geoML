...
| `models.py` | User-facing models: `GP` (legacy, closed-form), `VGPNetwork` (the core variational model), `StructuralField`, `GPEnsemble`, `ProjectedVGP`, `Normalizer`. Plus `refine(model, blocks, levels=...)`, a free function rather than a method because it is a workflow rather than something a model *is*: predict coarse, ask `needs_splitting`, cut, predict only what the cut made. |
