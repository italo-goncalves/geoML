...
| `likelihood.py` | Likelihoods (`Gaussian`, `MultivariateGaussian`, `Bernoulli`, `CategoricalGaussianIndicator`, `GradientIndicator`, …). Holds no reference to a data container — pure tensors. `_aggregate` averages a block's sub-blocks (25 call sites, so leave its signature alone) and `_dispersion` takes their variance over the same grouping; both are read *after* the warping is undone, so a block value and its spread are in the variable's own units. |
