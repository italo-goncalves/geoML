...
- **Tests:** `geoml/test/` is a runnable pytest suite — 204 tests in 11 files, ~3 min, entirely on synthetic data or the bundled `sample_data/` (nothing is downloaded, so `datasets.macpass()` is the one loader left uncovered). By area: `test_drillhole.py` (82 tests) covers desurveying, interval tables, compositing and conversion; `test_storage.py`, `test_containers.py` and `test_coordinates_variance.py` the `ArrayStore` backend and the batched-write contract every container owes the models; `test_persistence.py`, `test_variable_persistence.py` and `test_model_persistence.py` the Zarr and saved-model round-trips; `test_vgp.py` and `test_real_cases.py` build, train and predict end to end, the latter mirroring the real modelling cases (Jura, Arctic lake, sunspots, Walker Lake) at a few iterations. geoML isn't pip-installed here, so run from the repo root (so `import geoml` picks up the working tree) with an env that has TF/TFP. No pytest config or CI yet. Note: a model is only reproducible if the global NumPy/TF RNG is seeded *before construction* — parameter init isn't tied to `options.seed`.
- **Version:** `__init__.py` and `setup.py` both say `0.5.1` (changelog's latest). They drifted apart once — bump both together.
