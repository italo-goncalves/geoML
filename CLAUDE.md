...
| `plots/` | EDA and model figures (matplotlib). `Explorer` holds a container + a continuous/categorical choice (+ an optional model) and draws `histogram`/`pairs`/`pca`/`scene`, plus `training_curve`/`transformed_pairs`/`prediction_scatter`/`accuracy`/`grade_tonnage`; `plots/prepare.py` holds the arithmetic and imports no plotting library; `plots/style.py` is the palette and the `rc_context`. Only sub-package in the codebase — add it to `packages=` in `setup.py` when adding modules. |
