...
- **A variable that holds components does not copy what belongs to them.** `VectorVariable`, `CompositionalVariable` and the `RockTypeVariable` family build their components inside `__init__`, so anything a *component* owns — its `cutoffs`, and whatever is added next — is left behind by every method that rebuilds a variable: `from_variable`, `carry_to`, `copy_to`, the persistence round trip. Each has to carry it across by hand, and each has been fixed once for forgetting to. The failure is quiet: the rebuilt variable is structurally complete and merely does nothing, so **add a test with a vector variable, not only a scalar and a categorical one** — categorical components are `_Category`, which is a different class with different attributes, and testing it proves nothing about the graded case. **A composition is a fourth case, not a kind of vector variable**: `CompositionalVariable` overrides `update` and its parts are `_Component`, which overrides it again and reads different keys (`prediction`, not `average_sim`), so a column added to `ContinuousVariable.update` and to `VectorVariable.update` still never reaches a composition. `noise_variance` was shipped that way and only surfaced on real data. Test all four — `test_plots.py`'s `trained` and `trained_composition` fixtures are the pair for the graded ones. `dispersion` had been missing from a composition's parts since it was added, for exactly this reason, and went in beside `noise_variance`. Both `__getitem__`s now walk `_ZARR_ATTRS` instead of naming columns again (skipping the `None`s — a `_Component` has no latent space of its own), so subsetting is one place that maintains itself, and `_Component.as_data_frame` **delegates** to `ContinuousVariable`'s with `latent=False` rather than restating its columns (which is how it came to be missing `dispersion` and `noise_variance`) — the parent skips whatever is `None` or all-NaN, which is what lets one method serve both. **The rule to take from five bugs in this family: when a component's method is a copy of its parent's minus something, make the parent skip that something and delete the copy.** Exports name a component by its owner too — `assay - a - prediction`, `rock - granite - probability` — via `_export_label`; `BlockSet3D.get_contour` reads those labels back, so `_variable_or_component` returns the owner's name beside the component.
