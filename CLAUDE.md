...
- **`geoml/test/` is NOT a runnable suite.** The files are stale scratch scripts: they hardcode machine-specific paths (`C:\Dropbox\...`) and call removed APIs (`geoml.models.InterpolatingBands`, `geoml.data.Examples.walker()`, `p_real.value`). Do not assume they pass or reflect the current API — verify against source. If asked to add real tests, treat this as greenfield (no pytest config exists yet).
