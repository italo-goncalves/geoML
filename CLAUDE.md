...
| `geometry.py`, `metrics.py` | Rotation/angle helpers, plus the triangulated-surface maths behind `Surface3D` and the assignments (`weld`, `fan_triangulation`, `vertex_normals`, `open_edges`/`reversed_edges`/`signed_volume` — closed? consistently wound? facing out? — and `sheet_interpolator`/`sheet_elevation`, `inside_solid`) — arrays in, arrays out, no containers; scoring metrics. |
